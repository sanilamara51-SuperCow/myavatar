...
# 导入所有实现了的执行节点
from nodes.n1b_ppt_vision_scriptwriter import ppt_vision_scriptwriter_node
from nodes.n4_tts_synthesizer import tts_synthesizer_node
from nodes.n5_ffmpeg_assembler import ffmpeg_assembler_node

def build_ppt_driven_graph():
    """
    构建图像驱动的视频生成管线 (Image-Driven Workflow)
    流程: PPT图片输入 -> 视觉脚本生成 -> 语音合成 -> 视频混流组装
    """
    # 1. 实例化图并绑定状态 Schema
    workflow = StateGraph(VideoGenerationState)
    
    # 2. 注册图节点
    workflow.add_node("n1b_ppt_vision_scriptwriter", ppt_vision_scriptwriter_node)
    ...
    # 4. 定义数据流图边的流转结构
    workflow.add_edge(START, "n1b_ppt_vision_scriptwriter")
    
    add_safe_edge("n1b_ppt_vision_scriptwriter", "n4_tts_synthesizer")
